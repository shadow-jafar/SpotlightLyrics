...
final class VTKaraokeLyricLabel: UILabel {
    ...
    var lyricSegment:Dictionary<CGFloat,String>? {
        didSet {
            ...
            let sortedKeys = lyricSegment.keys.sorted()
            
            var fullText = ""
            for k in sortedKeys {
                
                if let segmentStr = lyricSegment[k] {
                    fullText = fullText.appendingFormat(segmentStr)
                }
    ...
    func prepareForLyricLabel() {
        ...
        textLayer.fontSize  = textFont?.pointSize ?? 0.0
    ...
    func keyTimesFromLyricSegment() -> Array<CGFloat> {
        ...
        let keyTimes:Array<CGFloat> = [0.0] + lyricSegment.keys.sorted() + [1.0]
        return keyTimes
    }
